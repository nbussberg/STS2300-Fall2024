...
1. Install the `moderndive` package on your computer if you haven't done so yet. Then add code above to load the package in this document. Explain below why we do not want to include the code to install the package in this document.
...
2. Write code below to bring up the help menu for the `mario_kart_auction` data set from the `moderndive` package. Then tell me how many observations are in the data and give an example of at least one quantitative variable and at least one categorical variable.
...
3. Use a function to display the number of games that are new vs. used. Then add another line of code that displays the proportion of games that are new vs. used.
...
7. Take your code from the previous question and make it so that a separate summary is calculated for new games and for used games. In other words, your `mario_summary` data frame will now have two rows (new and used). Is there more variation for number of bids among new games or used games?
...
9. Is each of the following an argument, function, object, or operator? Put your answer after the colon for each piece of code.
...
10. Write code to create a vector called `spring_24` that includes the courses you're taking this semester (e.g. STS2300).
...
11. Answer one (or both) of the following questions. Your answer should be at least 3 - 4 sentences long and demonstrate some level of reflection (i.e. not just a few words). Be sure to answer the full question as well.
  ...
  - What's something from Notes 01 - 02 that isn't entirely clear to you *and* what do you think would help to solidify this concept for you? Be specific.
